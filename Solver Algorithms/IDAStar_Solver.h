//
// Created by akshi on 20-06-2025.                          8 - Completed - 19
//
...
private:
    ...
    struct Node {
        ...
        Node(T _cube, const int _depth,int _estimate) : cube(_cube), depth(_depth), estimate(_estimate) {};
    };
...
//        priority_queue contains pair(Node, move done to reach that)
        ...
        while (!pq.empty()) {
            ...
            move_done[node.cube] = static_cast<Generic_Rubiks_Cube::MOVE>(p.second);
...
public:
    ...
    IDAStar_Solver(T _rubiksCube, const string &fileName) {
